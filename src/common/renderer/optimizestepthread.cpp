...
bool OptimizeStepThread::GetMergedNode(RendererNode **node) const
{
...
void OptimizeStepThread::AddToModel(QStandardItemModel *model, int row, int col) const
{
    if(spanFor) {
...
void OptimizeStepThread::UpdateView( QStandardItemModel *model ) const
{
    foreach(RendererNode *node, listOfNodes) {
        if(node) {
